...
class ImportMovieSynopsisCsvCommandTests(TestCase):
    def setUp(self):
        self.author = get_user_model().objects.create_user(
            username="synopsis_admin", email="synopsis@example.com", password="test1234"
        )

    def _write_csv(self, csv_content):
        temp_dir = TemporaryDirectory()
        csv_path = Path(temp_dir.name) / "synopsis.csv"
        csv_path.write_text(csv_content, encoding="utf-8")
        self.addCleanup(temp_dir.cleanup)
        return csv_path

    def _create_movie(self, imdb_id, synopsis=""):
        return Movie.objects.create(
            author=self.author,
            title_english=f"Movie {imdb_id}",
            title_spanish=f"Película {imdb_id}",
    ...
    def test_import_movie_synopsis_csv_updates_only_empty_synopsis(self):
        empty_movie = self._create_movie("tt1630029")
        existing_synopsis = "Already imported synopsis."
        movie_with_synopsis = self._create_movie("tt1375666", synopsis=existing_synopsis)
        csv_path = self._write_csv(
            "imdb_id;synopsis\n"
            "tt1630029;Jake Sully lives with his newfound family...\n"
            "tt1375666;This should not replace the current synopsis.\n"
            "tt0000000;Missing movie synopsis.\n"
            "tt1111111;\n"
        )
        out = io.StringIO()

        call_command("import_movie_synopsis_csv", str(csv_path), stdout=out, batch_size=2)

        empty_movie.refresh_from_db()
        movie_with_synopsis.refresh_from_db()
        self.assertEqual(empty_movie.synopsis, "Jake Sully lives with his newfound family...")
        self.assertEqual(movie_with_synopsis.synopsis, existing_synopsis)
        self.assertEqual(Movie.objects.count(), 2)

        output = out.getvalue()
        self.assertIn("Total filas leídas: 4", output)
        self.assertIn("Actualizadas: 1", output)
        self.assertIn("Saltadas porque ya tenían synopsis: 1", output)
        self.assertIn("No encontradas por imdb_id: 1", output)
        self.assertIn("Saltadas por synopsis vacía: 1", output)
        self.assertIn("Errores de lectura: 0", output)

    def test_import_movie_synopsis_csv_dry_run_does_not_save(self):
        movie = self._create_movie("tt1630029")
        csv_path = self._write_csv(
            "imdb_id;synopsis\n"
            "tt1630029;Dry-run synopsis.\n"
        )
        out = io.StringIO()

        call_command("import_movie_synopsis_csv", str(csv_path), stdout=out, dry_run=True)
    ...
    def test_import_movie_synopsis_csv_limit_and_start_row(self):
        skipped_movie = self._create_movie("tt0000001")
        first_processed_movie = self._create_movie("tt0000002")
        second_processed_movie = self._create_movie("tt0000003")
        csv_path = self._write_csv(
        ...
        first_processed_movie.refresh_from_db()
        second_processed_movie.refresh_from_db()
        self.assertEqual(skipped_movie.synopsis, "")
        self.assertEqual(first_processed_movie.synopsis, "Two.")
        self.assertEqual(second_processed_movie.synopsis, "")
        self.assertIn("Total filas leídas: 1", out.getvalue())

    def test_import_movie_synopsis_csv_overwrite_replaces_existing_synopsis(self):
        movie = self._create_movie("tt1630029", synopsis="Old synopsis.")
        csv_path = self._write_csv(
            "imdb_id;synopsis\n"
            "tt1630029;New synopsis.\n"
        )
        out = io.StringIO()
...
class SocialPrivacyAndFriendshipTests(TestCase):
    ...
    def test_public_profile_can_enable_friend_requests_restricted(self):
        ...
        self.assertTrue(self.user.profile.friend_requests_restricted)
        self.assertTrue(response.data["friend_requests_restricted"])

    def test_enabling_friend_requests_restricted_deletes_only_current_friendships(self):
        friend = get_user_model().objects.create_user(
            username="current_friend", email="current-friend@example.com", password="test1234"
        )
        follower = get_user_model().objects.create_user(
            username="privacy_follower", email="privacy-follower@example.com", password="test1234"
        )
        followed = get_user_model().objects.create_user(
            username="privacy_followed", email="privacy-followed@example.com", password="test1234"
        )
        blocked = get_user_model().objects.create_user(
            username="privacy_blocked", email="privacy-blocked@example.com", password="test1234"
        )
        Friendship.objects.create(
            requester=self.user,
            user1=self.user,
        ...
        UserVisibilityBlock.objects.create(owner=self.user, blocked_user=blocked)
        self.client.force_authenticate(user=self.user)
        ...
        self.assertTrue(Follow.objects.filter(follower=follower, following=self.user).exists())
    ...
    def test_disabling_friend_requests_restricted_does_not_restore_deleted_friendships_or_private_history(self):
        ...
        self.client.force_authenticate(user=self.user)
        self.client.patch(reverse("profile-privacy"), {"friend_requests_restricted": True}, format="json")

        response = self.client.patch(
            reverse("profile-privacy"),
            {"friend_requests_restricted": False},
            format="json",
        )
        ...
        future_request_response = self.client.post(
            reverse("friendship-request-create", kwargs={"username": self.user.username})
        )

        self.assertEqual(future_request_response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Friendship.between(self.user, friend).get().status, Friendship.STATUS_PENDING)

    def test_private_profile_cannot_enable_friend_requests_restricted(self):
        self.client.force_authenticate(user=self.private_user)
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.private_user.profile.refresh_from_db()
        self.assertFalse(self.private_user.profile.friend_requests_restricted)

    def test_switching_profile_to_private_clears_friend_requests_restricted(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.user.profile.refresh_from_db()
        self.assertEqual(self.user.profile.visibility, Profile.Visibility.PRIVATE)
        self.assertFalse(self.user.profile.friend_requests_restricted)

    def test_restricted_public_profile_rejects_new_friendship_requests(self):
...
class MovieListViewSearchAndFiltersTests(TestCase):
    ...
    def test_autocomplete_ranks_exact_then_complete_word_then_prefix(self):
        prefix = self._create_movie(
            "Dunamis",
            release_year=2025,
            external_rating=9.8,
            external_votes=900000,
        )
        complete_word = self._create_movie(
            "La Duna",
            release_year=2024,
            external_rating=9.5,
            external_votes=800000,
        )
        exact = self._create_movie(
            "Duna",
            release_year=1984,
            external_rating=6.3,
            external_votes=100,
        ...
        response = self.client.get(
            self.url,
            {"autocomplete": "true", "q": "duna", "limit": 10},
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        result_ids = [movie["id"] for movie in response.data["results"]]
        self.assertEqual(result_ids[:3], [exact.id, complete_word.id, prefix.id])
        self.assertIn(prefix.id, result_ids)

    def test_autocomplete_uses_external_rating_before_external_votes(self):
        higher_rating = self._create_movie(
            "Duna Roja",
            external_rating=9.0,
            external_votes=10,
        )
        more_votes = self._create_movie(
            "Duna Azul",
            external_rating=8.5,
            external_votes=100000,
        )
        ...
        result_ids = [movie["id"] for movie in response.data["results"]]
        self.assertLess(result_ids.index(higher_rating.id), result_ids.index(more_votes.id))

    def test_autocomplete_uses_external_votes_for_equal_ratings(self):
        fewer_votes = self._create_movie(
            "Duna Clara",
            external_rating=8.5,
            external_votes=100,
        )
        ...
        result_ids = [movie["id"] for movie in response.data["results"]]
        self.assertLess(result_ids.index(more_votes.id), result_ids.index(fewer_votes.id))
    ...
    def test_autocomplete_uses_release_year_after_external_rating_and_votes(self):
        ...
        self.assertNotIn("recency_score", qs.query.annotations)
...
class MovieWatchProvidersEndpointTests(TestCase):
    ...
    def test_returns_country_watch_providers_grouped_by_monetization_type(self, mock_get):
        ...
        self.assertEqual(
            ...
            "https://www.themoviedb.org/movie/27205-inception/watch?locale=CO",
        )
        self.assertIsNone(response.data["flatrate"][0]["direct_url"])
        self.assertIsNone(response.data["flatrate"][0]["affiliate_url"])
        self.assertIsNone(response.data["flatrate"][0]["monetized_url"])
        self.assertFalse(response.data["flatrate"][0]["is_clickable"])
    ...
    def test_applies_general_provider_link_for_provider_and_country(self, mock_get):
        ...
        self.assertEqual(provider["affiliate_url"], "https://affiliate.example/netflix-co")
            ...
            json=lambda: {
                "results": {
                    "CO": {
                        "link": "https://www.themoviedb.org/movie/27205-inception/watch?locale=CO",
                        "flatrate": [
                            {"provider_id": 339, "provider_name": "MovistarTV", "logo_path": "/movistar.jpg", "display_priority": 8},
                            {
                                "provider_id": 1855,
                                "provider_name": "Starz Apple TV Channel",
                                "logo_path": "/starz-apple.jpg",
                                "display_priority": 21,
                            },
                            {
                                "provider_id": 582,
                                "provider_name": "Paramount+ Amazon Channel",
                                "logo_path": "/paramount-amazon.jpg",
                                "display_priority": 24,
                            },
                            {
                                "provider_id": 633,
                                "provider_name": "Paramount+ Roku Premium Channel",
                                "logo_path": "/paramount-roku.jpg",
                                "display_priority": 28,
                            },
                            {"provider_id": 207, "provider_name": "The Roku Channel", "logo_path": "/roku.jpg", "display_priority": 27},
                        ],
                    }
                }
            },
...
class StreamingProviderLinkCommandTests(TestCase):
    ...
    def test_refresh_streaming_provider_links_preserves_landing_url_when_affiliate_exists(self):
        ...
        self.assertEqual(link.landing_url, "https://old.example/netflix")
        self.assertEqual(link.affiliate_url, "https://affiliate.example/netflix")
        self.assertIsNotNone(link.last_verified_at)

    def test_refresh_streaming_provider_links_updates_landing_url_when_affiliate_is_missing(self):
        link = StreamingProviderLink.objects.create(
            provider_id=8,
            provider_name="Netflix",
            country_code="CO",
    ...
    def test_seed_streaming_provider_links_includes_colombia_provider_variants(self):
        StreamingProviderLink.objects.create(
            provider_id=9999,
            provider_name="Roku",
            country_code="CO",
        ...
        for provider_id, (provider_name, landing_url) in expected_links.items():
            with self.subTest(provider_id=provider_id):
                links = StreamingProviderLink.objects.filter(provider_id=provider_id, country_code="CO")
                self.assertEqual(links.count(), 1)
                link = links.get()
                self.assertEqual(link.provider_name, provider_name)
                self.assertEqual(link.landing_url, landing_url)
                self.assertTrue(link.is_active)

        self.assertFalse(StreamingProviderLink.objects.filter(provider_id=9999, country_code="CO").exists())

    def test_refresh_streaming_provider_links_keeps_colombia_channel_mappings_clickable(self):
        call_command("refresh_streaming_provider_links")
        ...
        for provider_id, landing_url in expected_landing_urls.items():
            with self.subTest(provider_id=provider_id):
                link = StreamingProviderLink.objects.get(provider_id=provider_id, country_code="CO")
                self.assertEqual(link.landing_url, landing_url)
                self.assertEqual(link.affiliate_url, "")
...
class TMDbCreditsEndpointTests(TestCase):
    ...
    def test_person_detail_endpoint_maps_sam_neill_personal_fields(self):
        ...
        def fake_get_tmdb_json(path, params=None, timeout=None):
            ...
            if path == "/person/4783":
                return {
                    "id": 4783,
                    "name": "Sam Neill",
                    "profile_path": "/yIY7lJqX6RwcG3G7mxN7t6HwCZp.jpg",
                    "known_for_department": "Acting",
                    "gender": 2,
                    "birthday": "1947-09-14",
                    "deathday": None,
                    "place_of_birth": "Omagh, County Tyrone, Northern Ireland, UK",
                }
            if path == "/person/4783/external_ids":
                return {
                    "facebook_id": "samneillofficial",
                    "instagram_id": "samneilltheprop",
                    "twitter_id": "TwoPaddocks",
                }
            self.fail(f"Unexpected TMDb path: {path}")
        ...
        self.assertEqual(response.data["name"], "Sam Neill")
        self.assertEqual(response.data["known_for_department"], "Acting")
        self.assertEqual(response.data["gender"], {"code": 2, "label": "Male"})
        self.assertEqual(response.data["birthday"], "1947-09-14")
        self.assertIsNone(response.data["deathday"])
        self.assertEqual(response.data["place_of_birth"], "Omagh, County Tyrone, Northern Ireland, UK")
        self.assertEqual(response.data["facebook_url"], "https://www.facebook.com/samneillofficial")
        self.assertEqual(response.data["instagram_url"], "https://www.instagram.com/samneilltheprop")
        self.assertEqual(response.data["x_url"], "https://x.com/TwoPaddocks")
    ...
    def test_person_detail_endpoint_keeps_person_details_when_external_ids_fail(self):
        ...
        def fake_get_tmdb_json(path, params=None, timeout=None):
            ...
            if path == "/person/4783/external_ids":
                raise TMDbServiceError("external ids unavailable")
            self.fail(f"Unexpected TMDb path: {path}")
...
class DirectionalUserRestrictionVisibilityTests(TestCase):
    def setUp(self):
        ...
        self.pato = get_user_model().objects.create_user(username="PatoDonald", email="pato-directional@example.com", password="test1234")
        ...
        Friendship.objects.create(requester=self.julian, user1=self.julian, user2=self.pato, status=Friendship.STATUS_ACCEPTED)
    ...
    def test_restriction_is_directional_and_only_filters_visibility_not_relationships(self):
        ...
        self.assertTrue(Follow.objects.filter(follower=self.julian, following=self.peck).exists())
    ...
    def test_normal_search_hides_restricting_user_but_restriction_search_allows_reciprocal_restriction(self):
        ...
        privacy_search = self.client.get(reverse("profile-privacy-blocked-users"), {"q": "@Juli"})
        self.assertEqual(privacy_search.status_code, status.HTTP_200_OK)
        privacy_usernames = [item["username"] for item in privacy_search.data]
        self.assertIn("Julian", privacy_usernames)
        self.assertIn("JulianHernandez", privacy_usernames)
        ...
        self.assertTrue(Follow.objects.filter(follower=self.julian, following=self.peck).exists())
        self.assertTrue(Friendship.between(self.julian, self.peck).filter(status=Friendship.STATUS_ACCEPTED).exists())

    def test_mention_autocomplete_is_bilateral_without_changing_empty_or_text_search(self):
        self.client.force_authenticate(self.julian)
        ...
        filtered = self.client.get(reverse("friends-list"), {"search": "P"})
        self.assertEqual([item["username"] for item in filtered.data["results"]], ["PatoDonald"])
    ...
    def test_restricted_directed_comment_is_rejected_before_persistence_and_unrestricted_one_works(self):
        ...
        rejected = self.client.post(
            ...
            {"body": "hello @Peck", "mentioned_username": "Peck"},
            format="json",
        )
        ...
        allowed = self.client.post(
            reverse("movie-comments", kwargs={"pk": self.movie.id}),
            {"body": "hello @Dennisse", "mentioned_username": "Dennisse"},
            format="json",
        )
        self.assertEqual(allowed.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Comment.objects.filter(author=self.julian, target_user=self.dennisse).exists())

        UserVisibilityBlock.objects.filter(owner=self.julian, blocked_user=self.peck).delete()
        restored = self.client.get(reverse("friends-list"), {"search": "Peck"})

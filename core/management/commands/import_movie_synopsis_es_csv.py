...
REQUIRED_COLUMNS = {"imdb_id", "synopsis_es"}


class Command(BaseCommand):
    help = "Importa sinopsis en español desde un CSV local a Movie.synopsis_es usando Movie.imdb_id."

    def add_arguments(self, parser):
        parser.add_argument("csv_path", type=str, help="Ruta al archivo CSV de synopsis_es a importar.")
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--overwrite",
            action="store_true",
            help="Permite reemplazar synopsis_es existente. Por defecto no se sobrescribe.",
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(
                f"Iniciando importación CSV de sinopsis en español desde: {csv_path} "
        ...
        with csv_path.open("r", encoding="utf-8-sig", newline="") as csv_file:
            ...
            for data_row_number, row in enumerate(reader, start=1):
                ...
                try:
                    ...
                    if not synopsis_es:
                        stats["empty_synopsis_es"] += 1
                        continue
                    batch.append({"imdb_id": imdb_id, "synopsis_es": synopsis_es})
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Saltadas porque ya tenían synopsis_es: {stats['already_had_synopsis_es']}"
            )
        )
        self.stdout.write(self.style.SUCCESS(f"No encontradas por imdb_id: {stats['not_found']}"))
        self.stdout.write(self.style.SUCCESS(f"Saltadas por synopsis_es vacía: {stats['empty_synopsis_es']}"))
    ...
    def _flush_batch(self, rows, stats, dry_run, overwrite, dry_run_updated_movie_ids):
        ...
        movies_by_imdb_id = {
            movie.imdb_id: movie
            for movie in Movie.objects.filter(imdb_id__in=imdb_ids).only(
                "id", "imdb_id", "synopsis_es"
            )
        }
        movies_to_update = []
        ...
        for row in rows:
            ...
            if already_queued or (movie.synopsis_es and not overwrite):
                stats["already_had_synopsis_es"] += 1
                continue

            movie.synopsis_es = row["synopsis_es"]
        ...
        with transaction.atomic():
            Movie.objects.bulk_update(movies_to_update, ["synopsis_es"], batch_size=len(movies_to_update))
